...
describe('GetGameHandler', () => {
  it('returns phase, tasks, and players with roles', () => {
    ...
    expect(result?.tasks.every(t => !t.ready)).toBe(true)
    expect(result?.players).toEqual([
      { id: 'player-1', name: 'Alice', role: 'product-owner', assignedTaskId: undefined, hasActed: false },
      { id: 'player-2', name: 'Bob', role: 'developer', assignedTaskId: undefined, hasActed: false },
    ])
  })

  it('returns undefined when the game does not exist', () => {
    const repository = new InMemoryGameRepository()
    expect(new GetGameHandler(repository).handle('unknown-game')).toBeUndefined()
  })
})
